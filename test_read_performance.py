"""
test_read_performance.py — 读取变焦系统真实光线追迹性能

测试流程：
  步骤 1：连接 Zemax（extension 模式）
  步骤 2：调用 write_zoom_system 写入系统
  步骤 3：调用 read_real_performance 读取性能
  步骤 4：打印结果表格
  步骤 5：保存 zmx 文件
  步骤 6：断开连接
"""
...
def run_test():
    ...
    try:
        ...
        # 步骤 3：调用 read_real_performance 读取性能
        # ------------------------------------------------------------------ #
        step_header(3, "读取真实光线追迹性能")

        perf_data = None
        try:
            perf_data = bridge.read_real_performance(
                ZOOM_CONFIGS,
                effl_rows=opt_info['effl_rows'] if opt_info else None,
                totr_rows=opt_info['totr_rows'] if opt_info else None
            )
        ...
        # 步骤 4：打印结果表格
        # ------------------------------------------------------------------ #
        step_header(4, "性能结果汇总")
        if perf_data and perf_data['configs']:
            print()
            print(f"  {'配置':<16} {'目标EFL':>9} {'实际EFL':>9} {'偏差%':>8} "
                  f"{'RMS Spot(μm)':>30}")
            print(f"  {'':>16} {'':>9} {'':>9} {'':>8} "
                  f"{'轴上':>9} {'0.7F':>9} {'全场':>9}")
            print(f"  {'-'*72}")
            ...
            for cfg in perf_data['configs']:
                ...
                rms_strs = [fmt_val(r) for r in rms]
                # 补齐 3 个视场
                while len(rms_strs) < 3:
                    rms_strs.append('N/A')

                print(f"  {name:<16} {target:>9.3f} {actual_str:>9} {err_str:>8} "
                      f"{rms_strs[0]:>9} {rms_strs[1]:>9} {rms_strs[2]:>9}")
        else:
            print("  无性能数据可打印。")
        ...
        # 步骤 5：保存 zmx 文件
        # ------------------------------------------------------------------ #
        step_header(5, f"保存系统文件：{SAVE_PATH}")
        try:
